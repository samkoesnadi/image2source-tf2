...
from image2source.utils import load_json_from_path
from image2source.html_SXN_parser.parser import encode_2_sxn
...
def convert_and_write_all_datasets(annotations_path, filename):
    ...
    image_ids = [ann["image_id"] for ann in
                 annotations]  # add with .png, because the files are in png
    html_ids = [ann["html_id"] + ".html" for ann in
                annotations]  # add with .png, because the files are in png
    ...
    for i_seq, seq in enumerate(seqs):
        ...
        if len_seq < MAX_SEQ_LEN_DATASET:
            ...
            tensors = [append_tensors(j, i_seq, seq[j:MAX_SEQ_LEN_DATASET + j]) for j in
                       range(1, len_seq - MAX_SEQ_LEN_DATASET + 1)]
...
def get_all_datasets(filename):
    ...
    feature_description = {
        ...
        'sxn_token': tf.io.FixedLenSequenceFeature([], tf.int64, allow_missing=True,
                                                   default_value=0),  # sxn in token
    ...
    def convert_data(dataset):
        return load_image(dataset["image_id"] + ".png"), tf.cast(dataset["sxn_token"],
                                                                 tf.int32), tf.cast(dataset["pos"],
                                                                                    tf.int32)

    _datasets = raw_dataset.map(_parse_function,
                                num_parallel_calls=tf.data.experimental.AUTOTUNE)  # map the string to real data
